...
#define BIT_TO_UL(size) (1 + (size / sizeof(unsigned long)) + ((size & (sizeof(unsigned long) - 1)) != 0))
...
#define bitset_fill(_name, _bool) \
  do { \
    unsigned long _fill = (_bool) ? 0x0 - 1 : 0x0; \
    for (unsigned i = 1; i < BIT_TO_UL(_name.size); i++) _name.stuff[i] = _fill; \
  } while (0);

//TODO assert boundary
#define bitset_setbit(_name, _index, _bool) \
  do { \
    unsigned char _mask = 0x1 << (_index % sizeof(unsigned long)); \
    bitset_index_t _byte_index = (_index / sizeof(unsigned long)) + 1; \
    _name.stuff[_byte_index] = (_bool) ? (_name.stuff[_byte_index] | _mask) : (_name.stuff[_byte_index] & (~_mask)); \
  } while (0);

//TODO assert boundary
#define bitset_getbit(_name, _index) (((_name.stuff[(_index / sizeof(unsigned long))] & (0x1 << (_index % sizeof(unsigned long)))) == 0) ? 0 : 1)
